...
function Invoke-NewSection {
  ...
  $position = if ($parsed.Values.ContainsKey("position")) { [int]$parsed.Values["position"] } else { 100 }
...
function Invoke-DocsToolsMain {
  ...
  switch ($command) {
    ...
    "preview" {
      Invoke-DocsPreview -ResolvedRepoRoot $ResolvedRepoRoot
      return
    }
    "build" {
      Invoke-DocsBuild -ResolvedRepoRoot $ResolvedRepoRoot
      return
    }
    "check" {
